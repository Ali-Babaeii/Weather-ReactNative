...
export default function Details ( ) {
  ...
  const nextHours = data.hourly.map((item) =>
...
const mathRound = (Num) => {
 ...
 const renderItem = ({ item,index }) => (
   ...
   <View style={{padding:20,alignItems:'center'}}>
        ...
        <Image style={{height:38,width:38,marginVertical:8}} source={setCardIcon(nextIcons[index])}></Image>
 ...
 style={{width:'100%',height:'100%',marginTop:94,flex:1,borderRadius:24}}
     ...
     source={details}
> */}
        <View style={{flex:1,alignItems:'center',justifyContent: 'center',width:'100%',backgroundColor:'#5D50FE',borderTopLeftRadius:38,borderTopRightRadius:38}}>

<TouchableOpacity style={{backgroundColor: 'white',padding:10,top:-18,position:'absolute',
        borderRadius: 50,}}
        onPress={()=> 
...
<Text style={{color:'white',fontSize:38,fontWeight:'bold',marginTop:18}}>Monday</Text>
<Image style={{width:100,height:100,marginTop:8}} source={setIcon(currentConditions.weather[0].icon)}></Image>
<Text style={{ color: "white", fontSize: 52,marginTop:8 }}>{mathRound(currentConditions.temp)}&#xb0;</Text>

<Text>{useSelector((state)=>state.modalStatus)}</Text>
<View style={{flexDirection:'row',marginTop:8}}>
<Text style={{ color: "white", fontSize: 26,fontWeight:'600',paddingHorizontal:44 }}>{mathRound(minTemps)}&#xb0;</Text>
<Text style={{ color: "white", fontSize: 26,fontWeight:'600',paddingHorizontal:44 }}>{mathRound(maxTemps)}&#xb0;</Text>
</View>

<View style={{backgroundColor:'white',width:'90%',borderRadius:20,marginTop:32,bottom:16}}>
   <FlatList
   horizontal
        data={id}
